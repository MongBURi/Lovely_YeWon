...
namespace _231031_HelloCShap04_chap06
{
    public partial class Form1 : Form
    {
        ...
        {
            //int num = int.Parse(textBox1.Text);   // "(공백)"이나 "a(문자)"넣을시 오류 발생

            // num이라는 변수가 선언이 되고, 거기에 textBox1의 글자가 숫자로
            // 변환이 되어 들어가고, 잘못된 값이 들어갈 경우 num=0이 된다.
            // 참고로 비주얼스튜디오2015까지는 int.TryParse할 때
            // out int num이 안 되고, int num으로 먼저 변수 선언하고 나서
            // out num 이렇게 넣어야 했다.
            
            int num2;   // 2017 이전 스타일
            int.TryParse(textBox1.Text, out num2);  // out이랑 ref랑 비슷한 애(함수 2개값 리턴 가능)
            
            int.TryParse(textBox1.Text, out int num);   // 2017부터
        ...
        // Enable = true 상태에서 Inteval에 적힌 ms만큼 반복.
