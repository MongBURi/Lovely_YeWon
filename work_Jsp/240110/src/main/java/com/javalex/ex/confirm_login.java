...
public class confirm_login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// PasswordEncoder 객체 :: 비밀번호 파라미터 처리, 보안상 이게 더 안전함
		...
		if (dao.check_Account(id, pw) == true) {
			...
			response.sendRedirect("240110_2_welcomeLogin.jsp");			
		}
		else {
			System.out.println("로그인 실패");
			response.sendRedirect("240110_2_inputLogin.html");
		}		
	}
